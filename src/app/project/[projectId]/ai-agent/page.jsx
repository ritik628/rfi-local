    ...
    <ReactMarkdown 
      ...
      components={{
        ...
        blockquote: ({node, ...props}) => (
          <div className={`my-4 p-3 border-l-3 rounded-r-lg text-[12px] italic leading-relaxed ${isUser ? 'bg-white/10 border-white/30 text-white/80' : 'bg-muted/40 border-primary/30 text-muted-foreground'}`} {...props} />
        ),
        table: ({node, ...props}) => (
          <div className={`my-5 overflow-hidden rounded-lg border shadow-sm backdrop-blur-sm ${isUser ? 'bg-white/5 border-white/20' : 'bg-background/50 border-border'}`}>
        ...
        code: ({node, inline, ...props}) => (
          inline 
            ? <code className={`px-1.5 py-0.5 rounded text-[11px] font-mono ${isUser ? 'bg-white/20 text-white' : 'bg-muted text-primary'}`} {...props} />
            : <div className="my-3 p-3 bg-neutral-900 rounded-lg overflow-x-auto"><code className="text-[12px] font-mono text-neutral-100" {...props} /></div>
        ),
        strong: ({node, ...props}) => <strong className={`font-semibold ${isUser ? 'text-white' : 'text-foreground/90'}`} {...props} />,
      }}
    >
      {content}
    </ReactMarkdown>
...
export default function AIAgentPage() {
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: '⚠️ Connection error. Please verify the backend service is active and Azure AI credentials are configured.' 
      }]);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-background overflow-hidden relative">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-muted-foreground line-clamp-2 mt-0.5">{p.text}</div>
                          </div>
                        </button>
                      );
                    })}
              ...
              <div className="bg-card border border-border group-focus-within:border-primary/40 rounded-xl p-1 md:p-1.5 flex items-center gap-2 md:gap-3 transition-all shadow-xl shadow-foreground/5">
                ...
                <div className="flex items-center gap-1.5 md:gap-2 pr-1 md:pr-1.5 shrink-0">
                  {voiceSupported && (
                    ...
                    >
                      {listening ? <MicOff className="w-3.5 h-3.5 md:w-4 md:h-4" /> : <Mic className="w-3.5 h-3.5 md:w-4 md:h-4" />}
                    </button>
                  )}
                  <button 
                    onClick={() => send()}
      ...
      <style jsx global>{`
        .scrollbar-themed::-webkit-scrollbar { width: 6px; height: 6px; }
        .scrollbar-themed::-webkit-scrollbar-track { background: transparent; }
        .scrollbar-themed::-webkit-scrollbar-thumb { background: oklch(0.85 0 0); border-radius: 10px; }
        .scrollbar-themed::-webkit-scrollbar-thumb:hover { background: oklch(0.75 0 0); }
      `}</style>
    </div>
  );
}
