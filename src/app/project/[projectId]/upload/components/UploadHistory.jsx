  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
      <div className="px-6 py-4 border-b border-border bg-muted/20">
        <h3 className="text-xs font-semibold text-foreground uppercase tracking-widest flex items-center justify-between">
          3. Upload History
          ...
          return (
            <div key={f.id} className="p-4 flex items-start gap-3 hover:bg-muted/10 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-foreground truncate" title={f.filename}>{f.filename}</div>
                ...
                {f.error_msg && (
                  <p className="text-[10px] text-destructive mt-1.5 bg-destructive/5 px-2 py-1 rounded border border-destructive/10">
                    {f.error_msg}
                  </p>
                )}
              </div>
              <div className="flex flex-col items-end gap-2 shrink-0">
                <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[10px] font-semibold uppercase tracking-tight ${status.bg} ${status.color} ${status.border}`}>
