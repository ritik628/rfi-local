...
export default function UploadPage() {
  ...
  useEffect(() => {
    importDoneRef.current = importDone;
  }, [importDone]);
  ...
  const onDropPdf = useCallback((accepted) => {
    ...
    const newUrls = {};
    pdfs.forEach((f) => {
      newUrls[f.name] = _createBlobUrl(f);
    });
  ...
  const {
    ...
    getInputProps: getExcelInput,
    isDragActive: isDragExcel,
  } = useDropzone({
    onDrop: onDropExcel,
    multiple: false,
    accept: {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-background overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-12 scrollbar-themed">
        <div
          className={`max-w-[1400px] mx-auto grid gap-8 ${showPreview ? "lg:grid-cols-[400px,1fr] grid-cols-1" : "grid-cols-1 max-w-[800px]"}`}
        >
          ...
          <div className="space-y-6">
              ...
              {isPdfMode ? (
                <div className="space-y-4">
                  ...
                  >
                    <input {...getPdfInput()} />
                    <FileUp
                      className={`w-10 h-10 mx-auto mb-4 ${isDragPdf ? "text-primary" : "text-muted-foreground"}`}
                    />
                    <div className="text-[14px] font-medium text-foreground mb-1">
                      {isDragPdf
                        ? "Drop RFI PDFs here"
                        : "Drag & drop RFI PDFs, or click"}
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="max-h-40 overflow-y-auto space-y-1.5 pr-2 scrollbar-themed">
                        {pdfFiles.map((f, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-3 bg-muted/40 border border-border rounded-lg p-2.5"
                          >
                            <FileText className="w-4 h-4 text-muted-foreground shrink-0" />
                            <span className="flex-1 text-xs text-foreground truncate font-normal">
                              {f.name}
                            </span>
                      ...
                      <button
                        className="w-full bg-foreground text-background font-medium py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
                  ...
                  {(importing ||
                    ["running", "queued"].includes(importProg?.status)) && (
                    <div className="bg-muted/50 border border-border rounded-xl p-5 space-y-4">
                      <div className="flex justify-between items-center text-sm font-medium">
                        ...
                        <span className="text-muted-foreground">
                          {importProg?.done || 0} /{" "}
                          {importProg?.total || pdfFiles.length}
                        </span>
                      </div>
                    ...
                    <div className="bg-emerald-50/50 border border-emerald-200 rounded-xl p-5 space-y-4 text-center">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          {
                            label: "Matched",
                            val: importDone.matched,
                            color: "text-emerald-600",
                          },
                          {
                            label: "Unmatched",
                            val: importDone.unmatched,
                            color: "text-amber-600",
                          },
                          {
                            label: "Errors",
                            val: importDone.errors,
                            color: "text-destructive",
                          },
                        ].map((stat) => (
                          <div
                            key={stat.label}
                            className="bg-white border border-border/50 rounded-lg py-3 px-1"
                          >
                            <div
                              className={`text-xl font-medium ${stat.color}`}
                            >
                              {stat.val}
                            </div>
                      ...
                      {importDone.unmatched_refs?.length > 0 && (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-left">
                          <div className="flex items-center gap-2 text-amber-800 text-[11px] font-medium uppercase mb-2">
                          ...
                          <div className="max-h-24 overflow-y-auto space-y-1 scrollbar-themed">
                            {importDone.unmatched_refs.map((ref, i) => (
                              <div
                                key={i}
                                className="text-[10px] text-amber-700 bg-amber-100/50 px-2 py-1 rounded truncate"
                              >
                                {ref}
                              </div>
                            ))}
                      ...
                      <button
                        onClick={clearAllPdfs}
                        className="w-full bg-white border border-border text-foreground font-medium py-2.5 rounded-xl hover:bg-muted/30 transition-colors text-sm"
                      >
                        Import more PDFs
                      </button>
                    </div>
                ...
                >
                  ...
                  {uploading ? (
                    ...
                    <Table
                      className={`w-12 h-12 mx-auto mb-4 ${isDragExcel ? "text-primary" : "text-muted-foreground"}`}
                    />
                  )}
                  <div className="text-sm font-medium text-foreground mb-1">
                    {uploading
                      ...
                      : isDragExcel
                        ...
                        : "Drag & drop Excel, or click"}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    SS-RFI-Log.xlsx supported (max 50MB)
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-xl p-5 space-y-3">
              <h3 className="text-xs font-medium text-primary uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="text-[12px] text-foreground/70 space-y-2 leading-relaxed">
                {isPdfMode ? (
                  <ul className="list-disc pl-4 space-y-1">
                    <li>
                      Supports <span className="font-medium text-foreground">PlanGrid RFI Export</span> PDFs only.
                    </li>
                    <li>
                      Extraction:{" "}
                      <span className="font-medium text-foreground">RFI No, Response, Signed By, Date</span>.
                    </li>
                    <li>
                      Matching is performed by <span className="font-medium text-foreground">content content</span>,
                      not filename.
                    </li>
                  </ul>
                ) : (
                  <ul className="list-disc pl-4 space-y-1">
                    <li>
                      RFI Log must contain discipline sheets (CIVIL, MEP, etc).
                    </li>
                    <li>
                      Required:{" "}
                      <span className="font-medium text-foreground">RFI Ref, Subject, Location, Description</span>.
                    </li>
                    <li>
                      Maximum file size: <span className="font-medium text-foreground">50 MB</span>.
                    </li>
                  </ul>
                )}
              </div>
            ...
            <div className="min-w-0 h-full">
              <div className="bg-card border border-border rounded-xl shadow-lg flex flex-col h-full sticky top-8 max-h-[calc(100vh-8rem)]">
                ...
                <div className="p-4 border-b border-border bg-muted/10 flex items-center justify-between shrink-0">
                  ...
                  <div className="text-[10px] font-medium text-muted-foreground bg-muted px-2 py-1 rounded">
                ...
                <div className="flex items-center border-b border-border bg-card shrink-0">
                  ...
                  <div className="flex-1 flex gap-1.5 px-3 overflow-x-auto scrollbar-none py-3">
                    {fileResults.map((fr, i) => (
                      <button
                        ...
                        className={`shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-medium transition-all border ${
                          i === previewIdx
                  ...
                  <button
                    onClick={() =>
                      setPreviewIdx((i) =>
                        Math.min(fileResults.length - 1, i + 1),
                      )
                    }
                ...
                <div className="flex border-b border-border shrink-0">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 py-4 text-[10px] font-medium uppercase tracking-widest transition-all border-b-2 ${
                        previewTab === tab.id
                    ...
                    <div className="h-full overflow-y-auto p-8 space-y-8 scrollbar-themed">
                      ...
                      <div className="flex items-start justify-between gap-6 pb-6 border-b border-border">
                        <div className="min-w-0">
                          ...
                          <h4
                            className="text-lg font-medium text-foreground truncate mt-1"
                        ...
                        <div
                          className={`shrink-0 px-3 py-1 rounded-full text-[10px] font-medium uppercase tracking-widest border ${
                            currentFile.matched
                              ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                              : currentFile.error
                      ...
                      {currentFile.error && (
                        <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-4 flex gap-3">
                          ...
                          <p className="text-sm text-destructive font-normal leading-relaxed">
                            {currentFile.error}
                          </p>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-2 gap-8">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                            <RefreshCw className="w-3 h-3" /> RFI Reference
                          </label>
                          <div className="bg-muted/30 border border-border rounded-lg px-4 py-3 text-sm font-medium text-foreground">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="bg-muted/30 border border-border rounded-lg px-4 py-3 text-sm font-normal text-foreground leading-relaxed">
                      ...
                      <div className="space-y-4 pt-4">
                        <label className="text-[10px] font-black text-amber-600 uppercase tracking-[0.2em] flex items-center gap-2">
                          <AlertTriangle className="w-4 h-4" /> System Metadata
                          Validation
                        </label>
                        ...
                        <div className="bg-amber-50/50 border border-amber-200 rounded-xl overflow-hidden">
                          ...
                          <div className="divide-y divide-amber-100">
                            {[
                              ...
                              {
                                label: "Consultant",
                                value: currentFile.consultant,
                              },
                              { label: "Client", value: currentFile.client },
                              {
                                label: "Contractor",
                                value: currentFile.contractor,
                              },
                            ]
                              .filter((f) => f.value)
                              .map((item) => (
                                ...
                                >
                                  <div className="text-[11px] font-bold text-amber-900/60 uppercase">
                                    {item.label}
                                  </div>
                              ...
                              {Object.entries(mismatches).map(
                                ([field, val]) => {
                                  ...
                                  return (
                                    <div
                                      key={field}
                                      className="bg-card border border-amber-200 rounded-xl p-4 shadow-sm animate-in fade-in slide-in-from-right-4 duration-300"
                                    >
                                      <div className="flex items-center justify-between mb-3">
                                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                                          Mismatch: {MISMATCH_LABELS[field]}
                                        </span>
                                        <div className="flex gap-2">
                                          <button
                                            onClick={() =>
                                              handleIgnoreMismatch(field)
                                            }
                                      ...
                                      <div className="flex items-center gap-3">
                                        <div className="flex-1 bg-muted/50 p-2 rounded-lg text-xs border border-border">
                                          <div className="text-[8px] font-bold text-muted-foreground uppercase mb-0.5">
                                            Current
                                          </div>
                                        ...
                                        <div className="flex-1 bg-amber-50 p-2 rounded-lg text-xs border border-amber-200">
                                          <div className="text-[8px] font-bold text-amber-600 uppercase mb-0.5">
                                            Detected
                                          </div>
