  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
      <div className="px-6 py-4 border-b border-border bg-muted/20">
        <h3 className="text-[11px] font-medium text-foreground uppercase tracking-wider flex items-center justify-between">
          3. Upload History
          ...
          return (
            <div key={f.id} className="p-4 flex items-start gap-3 hover:bg-muted/10 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-normal text-foreground truncate" title={f.filename}>{f.filename}</div>
                <div className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-2">
                  <span>{new Date(f.created_at).toLocaleDateString()}</span>
                  {f.row_count > 0 && (
                    <span className="font-normal text-foreground/60 ml-2">
                      {f.row_count} records
                    </span>
                  )}
                </div>
              ...
              <div className="flex flex-col items-end gap-2 shrink-0">
                <div className={`flex items-center gap-1.5 px-2.5 py-0.5 rounded-full border text-[11px] font-medium uppercase tracking-tight ${status.bg} ${status.color} ${status.border}`}>
